...
import { createPost } from "@/app/server/admin/posts";
...
export default function CreatePostModal({
  ...
  // Load users on mount (only users with role "user")
  useEffect(() => {
    listUsers().then((result) => {
      if (result.success) {
        // Filter to only show users with role "user"
        setUsers(result.data.filter((user: any) => user.role === "user"));
      }
    });
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Auto-update status: if image is uploaded, set to pending; otherwise draft
      // Note: Server action creates as draft by default, but if image is provided,
      // we need to update it to pending after creation
      ...
      // If image was uploaded, update status to pending
      if (result.success && imageUrl && result.data) {
        const { updatePost } = await import("@/app/server/admin/posts");
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div>
            ...
            <select
              ...
              required
              className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-slate-900 bg-white"
            >
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <p className="text-sm text-blue-800">
              <strong>Note:</strong> New posts are created as <strong>draft</strong>. Status will automatically change to <strong>pending</strong> when the user uploads an image.
            </p>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-slate-200">
            ...
            >
              {isLoading ? "Creating..." : uploadingImage ? "Uploading..." : "Create Post"}
            </button>
          </div>
        </form>
      </div>
